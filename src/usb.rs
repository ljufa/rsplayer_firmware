                    ...
                    for &byte in &buf[..n] {
                        if byte == b'\n' {
                            if let Ok(received_str) = core::str::from_utf8(&received_data) {
                                ...
                                } else if received_str.starts_with("SetVU(")
                                    && received_str.ends_with(")")
                                {
                                    if let Some(content) = received_str
                                        .strip_prefix("SetVU(")
                                        .and_then(|s| s.strip_suffix(")"))
                                    {
                                            ...
                                            (parts.next(), parts.next())
                                        {
                                            if let (Ok(left), Ok(right)) =
                                                (l_str.parse::<u8>(), r_str.parse::<u8>())
                                            {
                                                control
                                                    .send(Command::UpdateVU { left, right })
                                                    .await;
                                            }
                                        }
                                    }
                                } else if received_str.starts_with("SetTrack(")
                                    && received_str.ends_with(")")
                                {
                                    if let Some(content) = received_str
                                        .strip_prefix("SetTrack(")
                                        .and_then(|s| s.strip_suffix(")"))
                                    {
                                        let mut parts = content.split('|');
                                ...
                                } else if received_str.starts_with("SetProgress(")
                                    && received_str.ends_with(")")
                                {
                                    if let Some(content) = received_str
                                        .strip_prefix("SetProgress(")
                                        .and_then(|s| s.strip_suffix(")"))
                                    {
                                        let mut parts = content.split('|');
                                        if let (Some(curr), Some(tot), Some(pct_str)) =
                                            (parts.next(), parts.next(), parts.next())
                                        {
                                ...
                                } else if received_str.starts_with("SetPlaybackMode(")
                                    && received_str.ends_with(")")
                                {
                                    if let Some(mode_str) = received_str
                                        .strip_prefix("SetPlaybackMode(")
                                        .and_then(|s| s.strip_suffix(")"))
                                    {
